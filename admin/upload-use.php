  ...
  <!-- header and slider end  -->
		  <?php
				if(isset($_POST['submit']))
				{
                    ...
                    $row = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM `howtouse` WHERE `product_id` ='$productid' "));
                    if($row >= 1){
                        echo "<script> alert('not possible');
                        window.location.href = 'list-use.php';
                        </script>";
                    }
                    else{
					move_uploaded_file($_FILES['Image']['tmp_name'], "img/".str_replace(" ","_",$_FILES['Image']['name']));
				    $query = mysqli_query($con,"INSERT INTO `howtouse` (`categoryid`,`subcategoryid`,`childcatid`,`product_id`,`image`) values('$category','$subcategory','$childcategory','$producti','$image') ");
					if($query)
					{
