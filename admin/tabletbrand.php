		...
		if(isset($_POST['watchbrand']))
		  {
			...
			foreach($name as $key => $value){
				$productimage = getimagesize($_FILES['Image']['tmp_name'][$key]);
	if(isset($productimage['mime'])){
	if($productimage['mime'] == "image/png"){
	$pimg = imagecreatefrompng($_FILES['Image']['tmp_name'][$key]);
    if(isset($pimg)){	
	 $out_img = time().random_int(100000, 999999).'.webp';
	 imagewebp($pimg,"img/".$out_img,100);
				    $query = mysqli_query($con,"INSERT INTO `subcategory` (`category_id`,`subcategory_name`,`subcategory_image`)
                     values('$cat_id','$value','$out_img') ");
					 }else{
			echo "<script> alert('image should be in png format');
                ...
                elseif($productimage['mime'] == "image/jpeg"){
                    	$pimg = imagecreatefromjpeg($_FILES['Image']['tmp_name'][$key]);
    if(isset($pimg)){	
	 $out_img = time().random_int(100000, 999999).'.webp';
	 imagewebp($pimg,"img/".$out_img,100);
				    $query = mysqli_query($con,"INSERT INTO `subcategory` (`category_id`,`subcategory_name`,`subcategory_image`)
			...
			echo "<script> alert('image should be in png format');
					...
					if($query)
					{
						echo "<script> alert(' subcategory insert successfully');
						 window.location.href = 'tablebrandlist.php';
						</script>";
					}
					else
					{
												...
												<div class="form-group row align-items-center">
													...
													<div class="col-lg-7 col-xl-6">
														<select name="category" id="" class="form-control form-control-modern">
													     <option value="3" selected>Sell Tablet</option>
														</select>
													</div>
												</div>
                                                    ...
                                                    <div class="row">
												     <div class="col-1">
													  ...
													  <div class="col-2">
														...
														<div class="col-2">
															<button class="btn btn-primary" width="100%" id="addtag">add Brand</button>
														</div>
...
$(document).ready(function(e){
	var html = '<div class="row my-3" id="rmtag"><div class="col-1"><label class=" control-label text-lg-right mb-0">Image </label></div><div class="col-3"><input type="file" class="form-control form-control-modern" name="Image[]" value="" required /></div><div class="col-2"><label class=" control-label text-lg-right mb-0">Brand Name</label></div><div class="col-4"><input type="text" class="form-control form-control-modern" name="Name[]" value="" required /></div><div class="col-2"><button class="btn btn-primary" id="removed">remove</button></div></div>';
