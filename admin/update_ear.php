...
if(isset($_POST['product']))
{
	...
	{
	$query = mysqli_query($con,"UPDATE `product` SET `categoryid`='$category',`subcategoryid`='$subcategory',`product_name`='$productname'  WHERE `id` = '$pid' ");
			if($query)
			 {
    ...
    if(isset($pimg)){	
	 ...
	 imagewebp($pimg,"img/".$out_img,100);
	   $query = mysqli_query($con,"UPDATE `product` SET `categoryid`='$category',`subcategoryid`='$subcategory',`product_name`='$productname',`product_image`='$out_img' WHERE `id` = '$pid' ");
		  if($query)
		  {
			echo "<script> alert('insert successfully');
				window.location.href = 'earlist.php';
